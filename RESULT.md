...
| Accumulation windows | 3, 6, 12, 24, 48, 72, 168 h |
...
On a non-cloud WAN, C2 is network-bound (~14 min/day for ~1.2 GB of GRIB
bytes). An application-level retry with backoff on the coalesced fetch recovers
transient S3 failures that otherwise dropped most files under 16-way
concurrency (25/30 → 0/30 failed files per day).
...
alarms held at 0–2. Detection lags onset by ~4 days — physically expected for
...
(876 rows, 358 events, 116 admin-1 units). The set contains zero negatives: the
local GPM archive is too sparse for non-event days to supply observations, so
they are dropped as missing. Refined tables trained on positives only would
over-escalate; `use_refined_cpts` is kept `false` in production. Activation
requires a dense GPM record to supply negatives.
